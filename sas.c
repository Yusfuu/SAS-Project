...
// print winner position of candidate
...
int main(int argc, char *argv[])
{
    ...
    // check if round 2  is duplacted min numbers
    int checkDup()
    {
        int dup = 0, min = 0, c;
        for (c = 1; c < numberOfCandidate; c++)
        {
            if (candidate[c].numberOfVotes < candidate[min].numberOfVotes)
            {
                min = c;
            }
        }
        for (c = min; c < numberOfCandidate; c++)
        {
            if (candidate[c].numberOfVotes == candidate[c + 1].numberOfVotes)
            {
                dup++;
            }
        }
    ...
    do
    {
        ...
        doVote();
        printf("\n%d", checkDup());
    } while (tie == numberOfCandidate - 1 || checkDup() > 0);
